"""Vendored from audio-separator's uvr_lib_v5/tfc_tdf_v3.py.

Kept structurally identical to upstream so the MDX23C DrumSep checkpoint's
state_dict loads with `strict=True`. The module is already torch-only and
beartype-free upstream, so it's copied verbatim with one addition:
`TFC_TDF_net.forward_spec`, the model body without the in-graph STFT/iSTFT
(the future ONNX-export cut point). `forward` keeps the full
STFT -> body -> iSTFT path for parity. STFT stays fp32.
"""
...
class TFC_TDF_net(nn.Module):
    ...
    def forward_spec(self, x):
        """Run the conv body on a post-STFT spectrogram.

        ONNX-export cut point: takes the STFT output `x` (shape
        `b (c*2) dim_f t`, the channels-as-complex representation produced by
        `STFT.__call__`) and returns the pre-iSTFT spectrogram of shape
        `b [n_instruments] (c*2) dim_f t`. Excludes the in-graph STFT/iSTFT.
        """
